...
$(BUILD)/epub/$(BOOKNAME).epub: $(CONTACT) $(TITLE) $(DIAGRAMS) $(CHAPTERS)
	mkdir -p $(BUILD)/epub
# 	the .epub target includes DIAGRAMS so the diagrams get included in the .epub
...
$(BUILD)/latex/$(BOOKNAME).tex: $(TITLE) $(CHAPTERS) $(CONTACT)
	mkdir -p $(BUILD)/latex
